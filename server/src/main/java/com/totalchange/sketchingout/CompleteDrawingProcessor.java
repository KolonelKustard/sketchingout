...
public class CompleteDrawingProcessor {
	private static final void sendMessage(Session session, String name,
			String email, int stage, DataSource imageDS, DataSource pdfDS)
			throws MessagingException, UnsupportedEncodingException {

		// Make new email
		SketchingoutEmail msg = new SketchingoutEmail(session,
				SketchingoutEmails.EMAILS);
		msg.setToName(name);
	...
	private static final void processMessages(Connection conn, String drawingID)
			throws CompleteDrawingProcessorException, SQLException {

		// Get the drawing from the database with all the contact details too.
		PreparedStatement pstmt = SQLWrapper
				.getCompleteDrawing(conn, drawingID);
		...
		try {
			// Check got a result
			if (!res.first()) {
				throw new CompleteDrawingProcessorException(
						"Could not find a drawing " + "with ID: " + drawingID);
			}

			// Find out how many stages are involved in this drawing
			int numStages = res.getInt("stage");

			// Create the drawing datasources that will serve as the attachment
			// to the outgoing email
			DrawingDataSource imageDS, pdfDS;
			try {
				imageDS = new DrawingDataSource(res.getInt("version"), res
						.getInt("width"), res.getInt("height"), 100,
						new BufferedImageParser("png"), "sketchingout.png",
						"image/png");

				pdfDS = new DrawingDataSource(res.getInt("version"), res
						.getInt("width"), res.getInt("height"), 100,
						new PdfImageParser(), "sketchingout.pdf",
						"application/pdf");

				// Now go through each stage and construct the drawing
				for (int num = 0; num < numStages; num++) {
					imageDS.addStage(res.getCharacterStream("stage_"
							+ (num + 1)));
					pdfDS
							.addStage(res.getCharacterStream("stage_"
									+ (num + 1)));
				}
			...
			// Have a drawing, so go through each stage again and send an email
			// with the drawing attached.
			for (int num = 0; num < numStages; num++) {
				...
				try {
					...
					// If an address that was supplied was invalid then a
					// messaging exception may be thrown. Don't want to return
					// this to the client, but want to log it somewhere so just
					// send it to standard err output.
					me.printStackTrace();
			...
			// And if have an additional recipient then chuck them on
			String additionalRecipient = res.getString("additional_recipient");
			if (additionalRecipient != null) {
				try {
					SketchingoutEmail email = new SketchingoutEmail(session,
							SketchingoutEmails.EMAILS_TO_ADDITIONAL_RECIPIENTS);
					email.setSenderName(res.getString("stage_" + numStages +
							"_author_name"));
					email.setSenderEmail(res.getString("stage_" + numStages +
							"_author_email"));
					email.setToName(res.getString("additional_recipient"));
					email.setToEmail(res.getString("additional_recipient"));
					email.send();
				} catch (MessagingException me) {
					// If an address that was supplied was invalid then a
					// messaging exception may be thrown. Don't want to return
